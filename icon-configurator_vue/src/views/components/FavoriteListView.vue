<template>
    ...
    <div class="row gy-3">
        <div v-for="(icon, index) in this.icons" :key="index" class="col-3 col-md-2">
            <icon-props @icon-removed="removeDeletedIcon()" :deleteIcons="this.deleteIcons" :refId="'icon_'+index" :editIdx="index" :editFavorites="true" :icon="icon.icon" :fill="icon.fill" :color="icon.color" :weight="icon.weight"></icon-props>
        </div>
...
export default{
  components: { IconProps },
    ...
    methods: {
        loadIcons(){
            setTimeout(() => {
                axios.get('http://localhost:3000/icons/getFavorites', { headers: authHeader() })
                .then(res => {
